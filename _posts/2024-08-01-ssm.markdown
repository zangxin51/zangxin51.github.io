---
layout: post
title: "SSM、Spring Boot 与 MyBatis-Plus 学习笔记"
subtitle: ""
...
# SSM、Spring Boot 与 MyBatis-Plus 学习笔记

本篇按技术演进顺序整理 Spring Framework、Spring MVC、Spring Boot 和 MyBatis-Plus，涵盖 IoC、DI、AOP、事务、REST、SSM 整合、配置管理、数据访问和代码生成。

原始课程参考：[SSM 框架学习视频](https://www.bilibili.com/video/BV1Fi4y1S7ix)

| 模块 | 核心内容 |
| --- | --- |
| Spring Framework | IoC、DI、Bean、注解开发、AOP、事务 |
| Spring MVC | 请求映射、参数绑定、响应、REST、异常处理、拦截器 |
| SSM 整合 | Spring、Spring MVC 与 MyBatis 的分层协作 |
| Spring Boot | Starter、自动配置、YAML、多环境配置、项目打包 |
| MyBatis-Plus | Mapper、条件构造器、分页、映射、逻辑删除、乐观锁 |
...
#### 在 resources 目录配置 applicationContext.xml
...
#### Controller层及表现层接口测试 (idea HttpClient)
...
![1631246806130](./../img/md-img/2024-08-01-ssm/1631246806130.png)
...
* 如果表在设计的时候有主外键关系，那么同时也得将合同表中的前三条数据也删除掉
  ...
  ![1631246997190](./../img/md-img/2024-08-01-ssm/1631246997190.png)
...
* 所以经过分析，我们不应该将表中的数据删除掉，而是需要进行保留，但是又得把离职的人和在职的人进行区分，这样就解决了上述问题，如:
  ...
  ![1631247188218](./../img/md-img/2024-08-01-ssm/1631247188218.png)
...
字段名可以任意，内容也可以自定义，比如`0`代表正常，`1`代表删除，可以在添加列的同时设置其**默认值为`0`正常。**

![1631247439168](./../img/md-img/2024-08-01-ssm/1631247439168.png)
...
![image-20240806175151788](./../img/md-img/2024-08-01-ssm/image-20240806175151788.png)
...
* 执行查询操作
  ...
  ![image-20240806175304111](./../img/md-img/image-20240806175304111.png)
...
![image-20240806175352419](./../img/md-img/image-20240806175352419.png)
...
![1631249913103](./../img/md-img/2024-08-01-ssm/1631249913103.png)
...
![1631252305080](./../img/md-img/2024-08-01-ssm/1631252305080.png)
...
![1631252393659](./../img/md-img/2024-08-01-ssm/1631252393659.png)
...
![1631252667865](./../img/md-img/2024-08-01-ssm/1631252667865.png)
...
![1631253302587](./../img/md-img/2024-08-01-ssm/1631253302587.png)
...
![1631253387845](./../img/md-img/2024-08-01-ssm/1631253387845.png)

## MyBatis-Plus 代码生成与 Service

### 代码生成器原理分析

造句:![1631253928893](./../img/md-img/2024-08-01-ssm/1631253928893.png)

我们可以往空白内容进行填词造句，比如:![1631253971409](./../img/md-img/2024-08-01-ssm/1631253971409.png)

在比如:![1631253994782](./../img/md-img/2024-08-01-ssm/1631253994782.png)

观察我们之前写的代码，会发现其中也会有很多重复内容，比如:

![1631254075651](./../img/md-img/2024-08-01-ssm/1631254075651.png)

那我们就想，如果我想做一个Book模块的开发，是不是只需要将红色部分的内容全部更换成`Book`即可，如：

![1631254119948](./../img/md-img/2024-08-01-ssm/1631254119948.png)
...
![1631254344180](./../img/md-img/2024-08-01-ssm/1631254344180.png)
...
![1631255110375](./../img/md-img/2024-08-01-ssm/1631255110375.png)

至此代码生成器就已经完成工作，我们能快速根据数据库表来创建对应的类，简化我们的代码开发。

### MyBatis-Plus Service 的 CRUD
