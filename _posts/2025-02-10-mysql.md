...
- 其他数据库支持CHECK, Oracle, SQL Server
...
而update,delete的时候, 产生的Undo Log日志不仅在回滚时需要, MVCC搬空访问时也需要, 不会被立即删除
...
MVCC是MySQL中多版本并发控制, 维护数据的多个版本, 使得读写操作没有冲突
...
- 权限名称
  ...
  - `alter`：修改表、库
  - `create`：创建表、库
  - `insert`、`delete`、`select`、`update` 等常用 DML/DQL 权限
...
grant all [privileges] on ... -- 表示该用户在该对象上的所有权限
```
...
- NEW/OLD
  ...
  MySQL 提供两个用来访问修改前后内容的变量：

  - `insert`：`NEW` 代表插入后的记录，`OLD` 无效。
  - `delete`：`OLD` 代表删除前的记录，`NEW` 无效。
  - `update`：`NEW` 代表修改后的内容，`OLD` 代表修改前的记录。
...
- 根据数据范围选择合适的数值类型，例如 `tinyint`、`int`、`bigint`。

- 字符串字段中，`char` 是定长类型，效率较高；`varchar` 是变长类型，空间更灵活。

- 索引优化见前文索引章节。

#### SQL 语句优化

- `select` 语句尽量指明字段名称，便于使用覆盖索引。

- SQL 语句要避免造成索引失效的写法。

- 尽量使用 `union all` 代替 `union`。`union` 会多一次去重过滤，效率较低。

- 避免在 `where` 子句中对字段做表达式操作，否则可能造成索引失效。
