...
这篇笔记整理一个微服务商城项目的练习过程，重点包括商品模块、图片上传、服务发现、配置中心、商品发布、搜索、Nginx 反向代理、负载均衡和动静分离。

| 模块 | 重点内容 |
| --- | --- |
| 环境搭建 | Docker Compose、MySQL、前后端脚手架、跨域配置 |
| 商品模块 | 分类、品牌、属性组、基本属性、销售属性、SPU、SKU |
| 微服务治理 | Aliyun OSS、Nacos 服务发现、网关、配置中心 |
| 前台页面 | 商城首页、分类展示、搜索结果页、Thymeleaf 渲染 |
| 部署优化 | Nginx 反向代理、负载均衡、动静分离 |
...
使用 Docker 环境, Docker compose-file
...
services:
  mysql:
    ...
    volumes:
      - ./docker_mount/jiaju_mall_v2/mysql-data:/var/lib/mysql
      - ./docker_mount/jiaju_mall_v2/myconf:/etc/mysql/conf.d
    restart: always
    networks:
      - jiaju_mall_v2_net
...
前端使用 renren-fast-vue 快速脚手架

使用 renren-fast 脚手架：https://gitee.com/renrenio/renren-fast

把配置文件中数据库改成自己的 MySQL 数据库

启动后台项目和前台项目, 依赖版本, 详见代码

解决跨域问题, 在前端 Vue 配置文件设置代理

代理的域名填后端 Tomcat 的启动地址
...
使用技术, Spring Boot, Vue 2, MyBatis-Plus
...
<img src="../img/md-img/2025-03-26-project-01/01.png" alt="截屏2025-03-26 08.57.50" />

接口：http://localhost:9090/commodity/category/list/tree

要在 parent-child 结构里显示 child 节点
...
注意点: 在实体类增加不存在于数据库的字段, 使用MyBatis-Plus的注解
...
逻辑删除：MyBatis-Plus 的 https://baomidou.com/guides/logic-delete/
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 08.58.37.png" alt="截屏2025-03-27 08.58.37" />
...
<img src="../img/md-img/截屏2025-03-27 08.48.05.png" alt="截屏2025-03-27 08.48.05" />
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 08.57.18.png" alt="截屏2025-03-27 08.57.18" />
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 12.25.38.png" alt="截屏2025-03-27 12.25.38" />
...
#### 3. 图片上传：使用 Aliyun OSS

1. 上传方式分析

普通上传方式: 用户数据需先上传到应用服务器，之后再上传到 OSS

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 14.35.57.png" alt="截屏2025-03-27 14.35.57" />

缺点: 速度慢, 应用服务器压力大, 应用服务器成本高

阿里云对象存储-服务端签名后直传

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 14.38.57.png" alt="截屏2025-03-27 14.38.57" />

Web端向服务端请求签名，然后直接上传，不会对服务端产生压力，而且安全可靠。但服务端无法实时了解用户上传了多少文件，上传了什么文件。如果想实时了解用户 上传了什么文件，可以采用服务端签名直传并设置上传回调

2. 在 Aliyun 创建 OSS bucket创建ram授权用户, 获取 OSS_ACCESS_KEY_ID和 OSS_ACCESS_KEY_SECRET

使用ossClient访问

Aliyun 文档 : https://help.Aliyun.com/zh/OSS/developer-reference/getting-started

集成Spring Cloudalibaba-OSS

https://github.com/alibaba/spring-cloud-alibaba/wiki/OSS

需求: 上传图片到 OSS, 使用对象存储-服务端签名后直传

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 19.56.22.png" alt="截屏2025-03-27 19.56.22" />

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 19.58.23.png" alt="截屏2025-03-27 19.58.23" />

注意点: aliyunOSS 上配置跨域访问, 配置公共读取权限, 否则不能显示

#### 4. 重构模块：引入服务发现和网关

现在模块有, mall.service(Aliyun 存储服务), mall.commodity(商品子模块), mall.admin(后台管理模块)

以及 Vue 管理台前端模块

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 20.28.13.png" alt="截屏2025-03-27 20.28.13" />

现在对这些服务进行统一 管理, 引入 Nacos 服务注册和发现中心

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-27 20.28.56.png" alt="截屏2025-03-27 20.28.56" />

创建mall.gateway模块

配置网关, 让admin Vue 前端走网关访问后端admin微服务
...
在网关mall-gateway服务统一配置跨域--->固定代码, 注意导包
...
后端必须校验, 否则利用postman等httpclient工具可以绕过前端校验
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 10.53.20.png" alt="截屏2025-03-29 10.53.20" />

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 10.53.43.png" alt="截屏2025-03-29 10.53.43" />
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 12.59.00.png" alt="截屏2025-03-29 12.59.00" />
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 15.37.35.png" alt="截屏2025-03-29 15.37.35" />

实现: elementUI的级联选择器控件, 复用之前的树形控件的接口, 分类id通过级联选择器给表单

需求3: 修改时回显所属分类id对应的联选择器

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 16.57.18.png" alt="截屏2025-03-29 16.57.18" />
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 20.24.42.png" alt="截屏2025-03-29 20.24.42" />

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 20.25.01.png" alt="截屏2025-03-29 20.25.01" />

需求2: 新增关联

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 20.25.51.png" alt="截屏2025-03-29 20.25.51" />

需求3: 解除关联

<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-29 20.27.02.png" alt="截屏2025-03-29 20.27.02" />

这个主要是前端复杂, 后端很简单, 单表 CRUD
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-30 13.17.21.png" alt="截屏2025-03-30 13.17.21" />
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-30 13.18.13.png" alt="截屏2025-03-30 13.18.13" />
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-03-30 13.16.40.png" alt="截屏2025-03-30 13.16.40" />
...
![截屏2025-03-30 17.14.42](../img/md-img/2025-03-26-project-01/截屏2025-03-30 17.14.42.png)

需求: 条件分页查询(attr_type=0), 新增, 修改, 删除

#### 11. 通过管理系统维护商品属性分组和基本属性关联
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-04-02 19.55.00-3594920.png" alt="截屏2025-04-02 19.55.00" />
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-04-02 21.12.39.png" alt="截屏2025-04-02 21.12.39" />

需求: 选择分类时, 品牌需要联动

前端 Vue 使用watch(监听属性变化)
...
![截屏2025-04-04 10.27.17](../img/md-img/2025-03-26-project-01/截屏2025-04-04 10.27.17.png)
...
![截屏2025-04-04 11.19.39](../img/md-img/2025-03-26-project-01/截屏2025-04-04 11.19.39.png)

需求: 分页+条件查询

#### 15. 商城首页（Thymeleaf）

<img src="../img/md-img/2025-03-26-project-01/截屏2025-04-05 10.31.26.png" alt="截屏2025-04-05 10.31.26" />

需求: 左边栏一级菜单显示, 二级菜单显示, 三级菜单显示

<img src="../img/md-img/2025-03-26-project-01/截屏2025-04-05 10.42.25.png" alt="截屏2025-04-05 10.42.25" />

```java
/**
    * 构造二级分类, 三级分类
    * Map<String,List<Catalog2Vo>>
    * map: catalog1Id --> List<Catalog2Vo>
    */
   @Override
   public Map<String, List<Catalog2Vo>> getCatalog2Map() {
       List<CategoryEntity> treeList = ListTree();
       Map<String, List<Catalog2Vo>> map = treeList.stream()
               .collect(Collectors.toMap(category -> category.getId().toString(), item1 -> {
                   System.out.println(item1.getId() + "#" + item1.getName());
                   List<CategoryEntity> child2Categories = item1.getChildCategories();
                   if (!CollectionUtils.isEmpty(child2Categories)) {
                       List<Catalog2Vo> catalog2VoList = child2Categories.stream().map(item2 -> {
                           System.out.println("\t"+item2.getId() + "##" + item2.getName());
                           Catalog2Vo catalog2Vo = new Catalog2Vo();
                           catalog2Vo.setCatalog1Id(item1.getId().toString());
                           catalog2Vo.setName(item2.getName());
                           catalog2Vo.setId(item2.getId().toString());
                           List<CategoryEntity> child3Categories = item2.getChildCategories();
                           if (!CollectionUtils.isEmpty(child3Categories)) {
                               List<Catalog2Vo.Catalog3Vo> catalog3VoList = child3Categories.stream().map(item3 -> {
                                   System.out.println("\t\t"+item3.getId() + "###" + item3.getName());
                                   Catalog2Vo.Catalog3Vo catalog3Vo = new Catalog2Vo.Catalog3Vo();
                                   catalog3Vo.setCatalog2Id(item2.getId().toString());
                                   catalog3Vo.setName(item3.getName());
                                   catalog3Vo.setId(item3.getId().toString());
                                   return catalog3Vo;
                               }).collect(Collectors.toList());
                               catalog2Vo.setCatalog3VoList(catalog3VoList);
                           }
                           return catalog2Vo;
                       }).collect(Collectors.toList());
...
<img src="../img/md-img/2025-03-26-project-01/截屏2025-04-05 13.39.12.png" alt="截屏2025-04-05 13.39.12" />

#### 16. 首页搜索

点击三级分类时进入搜索结果页面

<img src="../img/md-img/2025-03-26-project-01/截屏2025-04-05 13.40.57.png" alt="截屏2025-04-05 13.40.57" />

接口: http://localhost:9090/list.html?catalog3Id=707&keyword="显卡"
...
/**
 * 搜索数据: 条件+分页查询
 * url: http://localhost:9090/list.html?catalog3Id=717&limit=4&page=4
 * limit 每页容量
 * page 第几页
 */
...
public String searchList(@RequestParam Map<String, Object> params, Model model) {
    PageUtils pageUtils = skuInfoService.queryWebPage(params);
    SearchResultVo searchResultVo = new SearchResultVo();
    searchResultVo.setSkuList((List<SkuInfoEntity>) pageUtils.getList());
    searchResultVo.setTotal(pageUtils.getTotalCount());
    searchResultVo.setTotalPages(pageUtils.getTotalPage());
    searchResultVo.setPageNum(pageUtils.getCurrPage());
    // 构造分页导航条[1,2,3,4,5,..,]
    searchResultVo.setPageNavs(IntStream.range(1, pageUtils.getTotalPage() + 1).boxed().collect(Collectors.toList()));
    model.addAttribute("skuPage", searchResultVo);
    log.info("searchResultVo=\n{}", searchResultVo);
    return "list";
}
```

处理分页, 因为不是 Vue 的前后端分离, 所以还是需要整个页面更新, 不能使用局部ajax更新

能使用ajax, 但是需要用一大堆dom操作, 很难看

需求: 在之前的查询条件时, 需要判断商品的SPU是否为上架状态, 重构上面代码, 将封装数据的操作从controller中转移到servic里面

```java
// http://localhost:9090/list.html?keyword=显卡&catalog3Id=301&brandId=1
    // 增加 spu状态判断
    @Override
    public SearchResultVo queryWebPage(Map<String, Object> params) {
        String keyword = (String) params.get("keyword");
        String categoryId = (String) params.get("catalog3Id");
        String brandId = (String) params.get("brandId");

        // 只展示发布状态为上架的商品
        List<Long> statusUpIds = new LambdaQueryChainWrapper<>(spuInfoDao)
                .eq(SpuInfoEntity::getPublishStatus, 1)
                .list().stream()
                .map(SpuInfoEntity::getId)
                .collect(Collectors.toList());
        //  如果没有上架的商品,直接返回了
        if (CollectionUtils.isEmpty(statusUpIds)) {
            // 返回一个空数据
            SearchResultVo searchResultVo = new SearchResultVo();
            searchResultVo.setSkuList(new ArrayList<>());
            searchResultVo.setTotal(0L);
            searchResultVo.setTotalPages(0L);
            searchResultVo.setPageNum(1L);
            searchResultVo.setPageNavs(new ArrayList<>());
            return searchResultVo;
        }

        LambdaQueryWrapper<SkuInfoEntity> lw = new LambdaQueryWrapper<>();
        lw.eq(StringUtils.hasText(categoryId) && !categoryId.equals("0"), SkuInfoEntity::getCatalogId, categoryId)
                .eq(StringUtils.hasText(brandId) && !brandId.equals("0"), SkuInfoEntity::getBrandId, brandId)
                .and(StringUtils.hasText(keyword), item -> item.eq(SkuInfoEntity::getSkuId, keyword).or().like(SkuInfoEntity::getSkuName, keyword))
                .in(!CollectionUtils.isEmpty(statusUpIds), SkuInfoEntity::getSpuId, statusUpIds);
        ...
        SearchResultVo searchResultVo = new SearchResultVo();
        searchResultVo.setSkuList(page.getRecords());
        searchResultVo.setTotal(page.getTotal());
        searchResultVo.setTotalPages(page.getPages());
        searchResultVo.setPageNum(page.getCurrent());
        // 构造分页导航条[1,2,3,4,5,..,]
        searchResultVo.setPageNavs(LongStream.range(1L, page.getPages() + 1).boxed().collect(Collectors.toList()));
        return searchResultVo;
    }
```
...
<span class="page_span1">
    <a class="page_a" href="#" th:attr="pn=${skuPage.pageNum - 1}" th:if="${skuPage.pageNum>1}">
        < 上一页
    </a>
        <a class="page_a" href="#" th:each="nav:${skuPage.pageNavs}" th:text="${nav}"
           th:attr="pn=${nav},style=${nav == skuPage.pageNum?'border: 0;color:#ee2222;background: #fff':''}">1</a>
    <a class="page_a" href="#" th:attr="pn=${skuPage.pageNum + 1}" th:if="${skuPage.pageNum < skuPage.totalPages}">
        下一页 >
    </a>
</span>
        <span class="page_span2">
    <em>共<b th:text="${skuPage.totalPages}">10</b>页&nbsp;&nbsp;到第</em>
...
需求: 搜索时保留搜索关键字, 因为搜索结果刷刷新整个页面, 所以keyword无法再前端保存, 所以应该把keyword放在request域中, 即model中

```html
<input id="keyword_input" type="text" th:value="${skuPage.keyWord}" placeholder="请输入"/>
               <a href="#" onclick="searchByKeyword()">搜索</a>
```

在VO实体类中添加字段keyWord用来回显搜索关键字

至此业务功能已经完成

#### 17. 反向代理

现在配置 Nginx 相关的: 反向代理, 域名解析和动静分离

配置SSH登录linux

配置 Nginx, 卷挂载配置文件和挂载目录

```shell
docker run -d -p 80:80 -v /app/nginx_data:/usr/share/nginx -v jiaju_mall_nginx_conf:/etc/nginx --name jiaju_mall_nginx nginx:latest
```

ngx配置文件地址

/var/lib/Docker/volumes/jiaju_mall_nginx_conf/_data/conf.d/default.conf
...
```shell
192.168.2.85 www.jjmall.com
```

```nginx
location / {
  proxy_pass http://192.168.2.42:9090;
}
```

现在通过www.jjmall.com可以访问商城首页了

<img src="../img/md-img/2025-03-26-project-01/截屏2025-04-05 18.21.36.png" alt="截屏2025-04-05 18.21.36" />

#### 18. 负载均衡

配置负载均衡到网关, 请求-->Nginx-->网关-->微服务

在 Nginx.conf中配置网关

```nginx
upstream jiajuMallGateway {
    server 192.168.2.85:5050;
}
```

在conf.d/default.conf中配置

注意: 一定要配置proxy_set_header Host $host; 把host带上, 否则网关中的主机Host断言就使用不了

```nginx
location / {
  proxy_set_header Host $host;
  #proxy_pass http://192.168.2.42:9090;
  proxy_pass http://jiajuMallGateway;
}
```

网关(gateway微服务)配置商城首页

网关中host断言要放在最后面, 因为host优先级高于Path的优先级, 在Path里面用了路径重写, 访问会导致404---->没有路径重写, 所以host断言要放在最后面

```yaml
- id: host-router
  # 商城首页
  uri: http://192.168.2.42:9090
  predicates:
    # 只要主机是**.jjmall.com形式的就路由到http://192.168.2.85:9090
    - Host=**.jjmall.com
```
...
spring:
  ...
  cloud:
    nacos:
      discovery:
        #        server-addr: 192.168.2.42:8848
        server-addr: 192.168.2.85:8849

    gateway:
      routes:
        - id: commodity-router
          uri: http://192.168.2.42:9090
          #          uri: lb://mall-commodity 不用这个的原因就是,放在容器里面, 它使用容器的地址172.xxx.xx.xx的,物理机上的无法访问,
          # todo 以后再做处理,以前工作时处理方案是配置环境变量-宿主机的ip地址
          predicates:
            # 这个路径更加精确所以要放上面,不然访问不到
            - Path=/api/commodity/**
          filters:
...
在admin的前端代码中把写死的ip替换成www.jjmall.com, 也可以访问

#### 19. 动静分离

把commdity微服务的static目录的css, img, js, font, 等静态文件移动到 Nginx 中

```shell
/usr/share/nginx/html/static目录下
即宿主机/app/nginx_data/html/static
/app/nginx_data:/usr/share/nginx
```

```shell
scp -r <project-root>/mall.commodity/src/main/resources/static root@192.168.2.85:/app/nginx_data/html
```

重启commdity服务, 并清除浏览器缓存, 再去访问www.jjmall.com, 发现静态资源都没有了

<img src="../img/md-img/2025-03-26-project-01/截屏2025-04-05 20.02.04.png" alt="截屏2025-04-05 20.02.04" />

Nginx 配置静态资源转发路径 Nginx 的服务器文件目录
...
修改商城首页index.html和list.html中对资源路径的引用

比如: index/js/text.js 修改为 static/index/js/text.js

修改后重启commdity服务

解决点击三级分类时, 跳转显示出localhost:9090而不是域名问题
...
/list.html 点击超链接相当于host: port/list.html
